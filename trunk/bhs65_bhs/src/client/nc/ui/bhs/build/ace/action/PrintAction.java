...
import nc.vo.bhs.build.SoBuildOrderVO;
...
public class PrintAction extends MetaDataBasedPrintAction{
	...
	public Object[] getDatas() {
	    ...
	    for (int i = 0; i < datas.length; i++) {
			String csaleorderid = ((AggSoOrderBuildHVO)datas[i]).getParentVO().getCsaleorderid();
			...
			List<SoBuildOrderVO> moveorderList = new ArrayList<SoBuildOrderVO>();
			for (Iterator iterator = orderbidList.iterator(); iterator
					.hasNext();) {
				String string = (String) iterator.next();
				SoBuildOrderVO vo = new SoBuildOrderVO();
				vo.setCsaleorderbid(string);
				moveorderList.add(vo);
			}
			((AggSoOrderBuildHVO)datas[i]).setChildren(SoBuildOrderVO.class, moveorderList.toArray(new SoBuildOrderVO[0]));
