	...
	public Object[] getDatas() {
	    ...
	    if (this.getDataSplit() != null) {
	      ...
	      for (int i = 0; i < datas.length; i++) {
			String csaleorderid = ((AggSoMoveHVO)datas[i]).getParentVO().getCsaleorderid();
			...
			List<SoMoveOrderVO> moveorderList = new ArrayList<SoMoveOrderVO>();
			for (Iterator iterator = orderbidList.iterator(); iterator
					.hasNext();) {
				String string = (String) iterator.next();
				SoMoveOrderVO vo = new SoMoveOrderVO();
				vo.setCsaleorderbid(string);
				moveorderList.add(vo);
			}
			((AggSoMoveHVO)datas[i]).setChildren(SoMoveOrderVO.class, moveorderList.toArray(new SoMoveOrderVO[0]));
