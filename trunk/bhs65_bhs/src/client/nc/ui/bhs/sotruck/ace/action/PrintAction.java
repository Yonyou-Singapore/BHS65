...
import nc.vo.bhs.sotruck.SoTruckOrderVO;
...
public class PrintAction extends MetaDataBasedPrintAction{
	...
	public Object[] getDatas() {
	    ...
	    for (int i = 0; i < datas.length; i++) {
			String csaleorderid = ((AggSoTruckHVO)datas[i]).getParentVO().getCsaleorderid();
			...
			List<SoTruckOrderVO> moveorderList = new ArrayList<SoTruckOrderVO>();
			for (Iterator iterator = orderbidList.iterator(); iterator
					.hasNext();) {
				String string = (String) iterator.next();
				SoTruckOrderVO vo = new SoTruckOrderVO();
				vo.setCsaleorderbid(string);
				moveorderList.add(vo);
			}
			((AggSoTruckHVO)datas[i]).setChildren(SoTruckOrderVO.class, moveorderList.toArray(new SoTruckOrderVO[0]));
