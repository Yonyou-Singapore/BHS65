...
/**
 * 打开单据 
 * 2017-10-16 将truck 拆分成两个
 * @author Thinkpad 
 * 20170306
 * 
 */
public class SaleOrderSotruckAirAction extends SaleOrderBaseAction {

	private static final long serialVersionUID = -5751011383555490172L;

	public SaleOrderSotruckAirAction() {
		// TODO 自动生成的构造函数存根
		setCode("saleorderSotruckairaction");
		setBtnName("Truck-Air");
	}
	...
	public AbstractBill[] qryBackwardVO(SaleOrderHVO headvo) throws BusinessException {
		...
		//查询下游build的主键
		String sql = "select billid from bhs_sotruck_h where csaleorderid = '"+headvo.getCsaleorderid()+"' and isnull(dr,0) = 0 and transtypecode = 'TRUC-Cxx-air' ";
		...
		if(list == null || list.size() == 0){
			...
			vo.setVbillstatus(-1);
			vo.setPk_transtype("1001V810000000023476");
			vo.setTranstypecode("TRUC-Cxx-air");
