...
//		PeriodServiceFacade.checkDateScope(pk_org, periodCheckDate, periodCheckDate);
		PeriodServiceFacade.queryByDateWithCheck(pk_org, periodCheckDate);
		if(ArrayUtils.isEmpty(vos)){
			return vos;
		}
		LeaveTypeCopyVO typeVO = (LeaveTypeCopyVO) NCLocator.getInstance().lookup(ITimeItemQueryService.class).queryCopyTypesByDefPK(pk_org, pk_leavetype, TimeItemVO.LEAVE_TYPE);
		//排除掉无权计算的
		...
		Map<String, UFBoolean> perimssionMap = perimssionService.isUserhasPermissionByMetaDataOperation("60170psndoc", StringPiecer.getStrArrayDistinct(vos, LeaveBalanceVO.PK_TBM_PSNDOC), "CalcLeaveBalance", pk_group, pk_user);
...
//			没有计算权限的，刨除掉
	...
	protected LeaveBalanceVO calculate(String pk_org, LeaveTypeCopyVO typeVO,
			...
			throws BusinessException {
		return calculate(pk_org, typeVO, vo.getCuryear(), vo.getCurmonth(), new LeaveBalanceVO[]{vo}, calTime, needEnsure)[0];
	}
	/**
	 * @param pk_org
	 * @param typeVO
	 ...
	 * @param vos
	 * @param calTime
	 * @param withEnsure，true，表示传入的vos有可能数据库中还没有，需要ensure一下，false，表示vos肯定有，不需要ensure
	 * @return
	 * @throws BusinessException
	 */
	protected LeaveBalanceVO[] calculate(String pk_org, LeaveTypeCopyVO typeVO,
			String year, String month, LeaveBalanceVO[] vos, UFDateTime calTime,boolean needEnsure)
	throws BusinessException {
		if(ArrayUtils.isEmpty(vos)){
			return vos;
		}
		if(typeVO.getIslactation()!=null&&typeVO.getIslactation().booleanValue()){//如果是哺乳假，则不需要计算
			return vos;
//			throw new BusinessException("lactation holiday can not be computed!");
		}
		...
		try{
			...
			//		PeriodVO periodVO = NCLocator.getInstance().lookup(IPeriodQueryService.class).queryByYearMonth(pk_org, year, month);
...
//			InSQLCreator isc = null;
			...
			if(typeVO.getLeavesetperiod().intValue()==LeaveTypeCopyVO.LEAVESETPERIOD_DATE){
				for(LeaveBalanceVO vo:vos){
					...
					vo.setPeriodextendenddate(typeVO.getExtendDaysCount()==0?vo.getHireenddate():vo.getHireenddate().getDateAfter(typeVO.getExtendDaysCount()));
				}
			}
			else{
				...
				if(periodBeginEndDates!=null&&periodBeginEndDates.length==2){
					...
					UFLiteralDate periodExtendDate = typeVO.getExtendDaysCount()==0?periodEndDate:periodEndDate.getDateAfter(typeVO.getExtendDaysCount());
					for(LeaveBalanceVO vo:vos){
			...
			try{
				...
				if(!typeVO.getTimeitemcode().equals(TimeItemCopyVO.OVERTIMETOLEAVETYPE))
					calCurRealDayOrHour(pk_org, typeVO,year,month, periodBeginDate, periodEndDate,filteredVOs,inSQL,calTime);
				else
					balanceDAO.syncFromDB4DataExists(filteredVOs);//如果是加班转调休，则享有和实际享有也要从数据库中同步一下，防止后台和界面不一致
				calYiRestFreezeDayOrHour(pk_org, typeVO, year,month, periodBeginDate, periodEndDate, filteredVOs, inSQL, calTime);
				...
				if(LeaveTypeCopyVO.LEAVESETPERIOD_DATE == typeVO.getLeavesetperiod().intValue()){
					new BaseDAO().updateVOArray(filteredVOs, new String[]{LeaveBalanceVO.HIREBEGINDATE,LeaveBalanceVO.HIREENDDATE});
				}
			}
			finally{
				isc.clear();
	...
	public LeaveBalanceVO[] save(LoginContext context, String pk_leavetype,
		...
		LeaveBalanceVO[] oldvos = getServiceTemplate().queryByPks(LeaveBalanceVO.class, StringPiecer.getStrArray(vos, LeaveBalanceVO.PK_LEAVEBALANCE));
		LeaveTypeCopyVO typeVO =  (LeaveTypeCopyVO) NCLocator.getInstance().lookup(ITimeItemQueryService.class).queryCopyTypesByDefPK(context.getPk_org(),pk_leavetype,TimeItemCopyVO.LEAVE_TYPE);
//		int leavesetperiod = typeVO.getLeavesetperiod().intValue();
//		boolean isYear = leavesetperiod==LeaveTypeCopyVO.LEAVESETPERIOD_YEAR;//是否按年计算
		...
		for(int i=0;vos!=null&&i<vos.length;i++)
		{
			...
			//默认值
			if(vos[i].getIsabnormalset()==null)vos[i].setIsabnormalset(UFBoolean.FALSE);

			if(vos[i].getLastdayorhour()==null)vos[i].setLastdayorhour(UFDouble.ZERO_DBL);
			if(vos[i].getCurdayorhour()==null)vos[i].setCurdayorhour(UFDouble.ZERO_DBL);
			if(vos[i].getRealdayorhour()==null)vos[i].setRealdayorhour(UFDouble.ZERO_DBL);
			if(vos[i].getYidayorhour()==null)vos[i].setYidayorhour(UFDouble.ZERO_DBL);
			if(vos[i].getRestdayorhour()==null)vos[i].setRestdayorhour(UFDouble.ZERO_DBL);
			if(vos[i].getFreezedayorhour()==null)vos[i].setFreezedayorhour(UFDouble.ZERO_DBL);
	 ...
	 * @throws BusinessException
	 */
	private LeaveFormulaCalParam createFormulaPara(String pk_org, LeaveTypeCopyVO typeVO,String year,String month,UFLiteralDate calDate,
		...
		para.setPreviousCalYearPeriods(PeriodServiceFacade.queryByYear(pk_org, Integer.toString(Integer.parseInt(year)-1)));
	...
	protected void calCurRealDayOrHour(String pk_org, LeaveTypeCopyVO typeVO,String year,String month,
			UFLiteralDate periodBeginDate,UFLiteralDate periodEndDate, LeaveBalanceVO[] vos,String inSQL,UFDateTime calculateTime)throws BusinessException{
		InputStream is  =null;
		try{
			...
			" set "+LeaveBalanceVO.CURDAYORHOUR+"= isnull("+parsedFormula + ",0)";
			if(isZero)
			...
			if(CollectionUtils.isNotEmpty(transList))
				dao.updateVOArray(transList.toArray(new LeaveBalanceVO[0]),new String[]{LeaveBalanceVO.CURDAYORHOUR});
			...
			if(typeVO.isRealEqualsCur()&&!isZero){
				String updateSQL2 = "update "+LeaveBalanceVO.getDefaultTableName()+
				" set "+LeaveBalanceVO.REALDAYORHOUR+"="+LeaveBalanceVO.CURDAYORHOUR+where;
			...
			balanceDAO.syncFromDB4DataExists(vos);
			calRealDayORHour(pk_org, typeVO,year,month, periodBeginDate,periodEndDate, vos, inSQL, calculateTime, calDate);
		} catch (IOException e) {
	...
	/**
	 * 计算当前实际享有时长。
	 * 使用折算法，大致逻辑是，实际享有时长=(length(later(期间第一天，入职日),earlier(期间最后一天，计算日)))/length(整期间)*享有
	 * 对于按入职日结算的情况，期间第一天不可能比入职日早
	 * @param pk_org
	 * @param typeVO
	 * @param year
	 * @param month
	...
	protected void calRealDayORHour(String pk_org, LeaveTypeCopyVO typeVO,String year,String month,
		...
		BaseDAO dao = new BaseDAO();
		if(setPeriod==LeaveTypeCopyVO.LEAVESETPERIOD_YEAR&&periodBeginDate!=null&&calculateDate.before(periodBeginDate)){
			String updateSQL="update "+LeaveBalanceVO.getDefaultTableName()+" set "+LeaveBalanceVO.REALDAYORHOUR+"=0.0"+
			" where "+LeaveBalanceVO.PK_LEAVEBALANCE+" in ("+pkInSQL+")";
			dao.executeUpdate(updateSQL);
			LeaveBalanceDAO balanceDAO = new LeaveBalanceDAO();
			balanceDAO.syncFromDB4DataExists(vos);
			return;
		...
		if(setPeriod==LeaveTypeCopyVO.LEAVESETPERIOD_YEAR){
			...
			calRealDayORHour4YearPeriod(pk_org, typeVO, periodBeginDate,periodEndDate, vos, pkInSQL, calculateTime, calculateDate, hireDateMap);
			return;
		}
		//按入职日结算
...
//			查询出此期间内的所有审批通过的单据和登记单据，
			...
			LeaveRegVO[] regVOs = LeaveServiceFacade.queryByPsnsLeaveTypePeriod(pk_org,pk_psnorgs , typeVO, year, month,leaveindex);
			//查询出此期间内的自由态，审批中的申请单
			LeavebVO[] leavebVOs = LeaveServiceFacade.queryBeforePassWithoutNoPassByPsnsLeaveTypePeriod(pk_org, pk_psnorgs, typeVO, year, month, leaveindex);
//			LeaveCommonVO[] leaveVOs = CommonUtils.merge2Array(regVOs, leavebVOs);
...
//			Map<String, LeavebVO[]> leavebMap = CommonUtils.group2ArrayByField(LeavebVO.PK_PSNORG, leavebVOs);
			...
			for(LeaveBalanceVO vo:indexvos){
				LeaveRegVO[] leaveRegVOs = CollectionUtils.isEmpty(regMap.get(vo.getPk_psnorg()))?null:regMap.get(vo.getPk_psnorg()).toArray(new LeaveRegVO[0]);
				LeavebVO[] leavebVOs2 = CollectionUtils.isEmpty(leavebMap.get(vo.getPk_psnorg()))?null:leavebMap.get(vo.getPk_psnorg()).toArray(new LeavebVO[0]);
//				double result = BillProcessHelperAtServer.calConsumedLeaveLength(leaveRegVOs, pk_org, typeVO, timeRuleVO, billMutexRule, aggShiftMap);
				double result = getSumValue(leaveRegVOs);
				vo.setYidayorhour(result==0?UFDouble.ZERO_DBL:new UFDouble(result));
				//结余=上期结余+当前实际享有-已休
//				vo.setRestdayorhour(new UFDouble(vo.getLastdayorhour().doubleValue()+vo.getRealdayorhour().doubleValue()-result));
				//根据港华需求：结余=上期结余+当前实际享有-已休 + 调整时长
				double changeLength = vo.getChangelength() == null?0.0:vo.getChangelength().doubleValue();
				vo.setRestdayorhour(new UFDouble(vo.getLastdayorhour().doubleValue()+vo.getRealdayorhour().doubleValue()-result + changeLength));
...
//				if(typeVO.getIsleavetransfer()!=null&&typeVO.getIsleavetransfer().booleanValue()){
				...
				//计算冻结时长
//				double freeze = BillProcessHelperAtServer.calFreezeLeaveLength(leavebVOs2, pk_org, typeVO, timeRuleVO, billMutexRule, aggShiftMap);
		...
		//优化时删除，采用上面的方法分组处理
//		for(LeaveBalanceVO vo:vos){
//			double result = BillProcessHelperAtServer.calConsumedLeaveLength(pk_org, vo.getPk_psnorg(), typeVO,
//					year, month,vo.getLeaveindex(), timeRuleVO, billMutexRule, aggShiftMap);
//			vo.setYidayorhour(result==0?UFDouble.ZERO_DBL:new UFDouble(result));
//			//结余=上期结余+当前实际享有-已休
//			vo.setRestdayorhour(new UFDouble(vo.getLastdayorhour().doubleValue()+vo.getRealdayorhour().doubleValue()-result));
//			//计算冻结时长
//			double freeze = BillProcessHelperAtServer.calFreezeLeaveLength(pk_org, vo.getPk_psnorg(), typeVO, year, month, vo.getLeaveindex(), timeRuleVO, billMutexRule, aggShiftMap);
//			vo.setFreezedayorhour(new UFDouble(freeze));
//			vo.setCalculatetime(calculateTime);
//		}
		new BaseDAO().updateVOArray(vos, new String[]{LeaveBalanceVO.TRANSLATEIN,LeaveBalanceVO.TRANSLATEOUT,LeaveBalanceVO.YIDAYORHOUR,LeaveBalanceVO.RESTDAYORHOUR,LeaveBalanceVO.FREEZEDAYORHOUR,LeaveBalanceVO.CALCULATETIME});
	 ...
	 * @throws BusinessException
	 */
	protected void calRealDayORHour4YearPeriod(String pk_org, LeaveTypeCopyVO typeVO,
			UFLiteralDate periodBeginDate,UFLiteralDate periodEndDate, LeaveBalanceVO[] vos,String pkInSQL,
			UFDateTime calculateTime,UFLiteralDate calculateDate,Map<String, UFLiteralDate> hireDateMap)throws BusinessException{
		//根据港华的需求进行变更了
		double monthDiff = !calculateDate.before(periodEndDate)?12:(calMonthDiff(periodBeginDate, calculateDate));
		if(typeVO.getLeavesetperiod() == TimeItemCopyVO.LEAVESETPERIOD_YEAR && typeVO.getLeavescale() ==  TimeItemCopyVO.LEAVESCALE_MONTH){
			IPeriodQueryService periodSer = NCLocator.getInstance().lookup(IPeriodQueryService.class);
			PeriodVO culPeriod = periodSer.queryByDate(pk_org, calculateDate.before(periodEndDate)?calculateDate:periodEndDate);
	 ...
	 * @throws BusinessException
	 */
	protected void calRealDayORHour4HirePeriod(String pk_org, LeaveTypeCopyVO typeVO,
			String year, LeaveBalanceVO[] vos,String pkInSQL,
			UFDateTime calculateTime,UFLiteralDate calculateDate)throws BusinessException{
		for(LeaveBalanceVO vo:vos){
			...
			int psnMonthDiff = !calculateDate.before(periodEndDate)?12:(calMonthDiff(periodBeginDate,calculateDate));
			psnMonthDiff = Math.max(0, psnMonthDiff);
			psnMonthDiff = Math.min(12, psnMonthDiff);
			if(psnMonthDiff==12){
				vo.setRealdayorhour(vo.getCurdayorhour());
	...
	/**
	 * 根据入职日期、年度，计算出入职年的第一天
	 * 例如，入职日期是2008-03-03，那么2011年的入职年的第一天就是2011-03-03
	 * @param year
	 * @param hireDate
	 * @return
	 */
	...
	/**
	 * 根据入职日期、年度，计算出入职年的最后一天
	 * 例如，入职日期是2008-03-03，那么2011年的入职年的最后一天就是2012-03-02
	 * @param year
	 * @param hireDate
	 * @return
	 */
	protected UFLiteralDate getHireEndDate(String year,UFLiteralDate hireDate){
	    ...
	    if(!UFLiteralDate.isLeapYear(Integer.valueOf(year)+1)&& "-02-29".equals(hireDateStr.substring(4, hireDateStr.length()))){
	    	return UFLiteralDate.getDate((Integer.parseInt(year)+1)+"-"+hireDate.getDateBefore(1).toString().substring(4, (hireDate.getDateBefore(1).toString()).length())).getDateBefore(1);
	    }else{
	    	return UFLiteralDate.getDate((Integer.parseInt(year)+1)+"-"+hireDateStr.substring(4, hireDateStr.length())).getDateBefore(1);
	    }
	...
	protected UFLiteralDate getDateInYear(String year,UFLiteralDate date){
	    ...
	    if(!UFLiteralDate.isLeapYear(Integer.valueOf(year))&& "-02-29".equals(dateStr.substring(4, dateStr.length()))){
	    	return UFLiteralDate.getDate(year+"-"+date.getDateBefore(1).toString().substring(5, (date.getDateBefore(1).toString()).length()));
	    }else{
	...
	/**
	 * 查询一批人员的入职日期，
	 * @param psnorgInSQL，pk_psnorg的insql
	 * @return
	 * @throws BusinessException
	 */
	 ...
	 * 反结算
	 * 需要校验：
	 * 如果下期已经结算，或者存在同期大leaveindex的情况，则不能反结算
	 * 即，可以反结算的记录，应该满足条件：
	 * 1.下期未结算，且
	 * 2.不存在同期更大的leaveindex记录
	 *  (non-Javadoc)
	 * @see nc.itf.ta.ILeaveBalanceManageMaintain#unSettlement(java.lang.String, java.lang.String, java.lang.String, java.lang.String, nc.vo.ta.leavebalance.LeaveBalanceVO[])
	 */
	@Override
	public UnSettlementResult unSettlement(String pk_org, String pk_leavetype,
		...
		String badCond = "pk_org=? and pk_psnorg=? and pk_timeitem=? and (("+periodField+">? and "+LeaveBalanceVO.ISSETTLEMENT+"='Y') or ("+periodField+"=? and "+LeaveBalanceVO.LEAVEINDEX+">?))";
		String nextPeriodCond = "pk_org=? and pk_psnorg=? and pk_timeitem=? and "+periodField+"=? and "+LeaveBalanceVO.LEAVEINDEX+"=1";
...
//				errMsg.append(MessageFormat.format(getNextPeriodSealedError(), Integer.toString(lineNoMap.get(vo.getPk_psnorg()+vo.getLeaveindex())+1)));
				continue;
			}
			...
			if(leaveSet==LeaveTypeCopyVO.LEAVESETTLEMENT_NEXT){
				...
				LeaveBalanceVO[] nextPeriodVOs = CommonUtils.retrieveByClause(LeaveBalanceVO.class, nextPeriodCond, para);
				...
				if(!ArrayUtils.isEmpty(nextPeriodVOs)){
					...
					if(vo.getRestdayorhour()!=null&&vo.getRestdayorhour().doubleValue()!=0){//如果本期结余不为0，才有必要重算下期的结余
						updateVO.setRestdayorhour(updateVO.getRestdayorhour()==null?UFDouble.ZERO_DBL.sub(vo.getRestdayorhour()):updateVO.getRestdayorhour().sub(vo.getRestdayorhour()));
					}
				}
			}
	...
	private LeaveBalanceVO[] filterCanUnSettlementVOs(LeaveBalanceVO[] vos) throws BusinessException{
		...
		String pk_group = InvocationInfoProxy.getInstance().getGroupId();
		IDataPermissionPubService perimssionService = NCLocator.getInstance().lookup(IDataPermissionPubService.class);
		Map<String, UFBoolean> perimssionMap = perimssionService.isUserhasPermissionByMetaDataOperation("60170psndoc",StringPiecer.getStrArrayDistinct(vos, LeaveBalanceVO.PK_TBM_PSNDOC), "UnLeaveBalanceAction", pk_group, pk_user);
		for(LeaveBalanceVO vo:vos){
...
//				continue;
	...
	/**
	 * 适用于按入职日结算的查询
	 * 要求的查询结果是：如果pk_psnorg在year的入职年范围内与考勤档案有交集，则至少有一条结算记录
	 * 如果数据库中还没有，则应该在内存中生成一条
	 * 如果数据库中有，且存在未结算的，则返回数据库中的
	 ...
	 * @throws BusinessException
	 */
	protected LeaveBalanceVO[] queryByCondition4HireDateSettlement(String pk_org,
			LeaveTypeCopyVO typeVO, String year, FromWhereSQL fromWhereSQL)
	throws BusinessException {
		...
		TBMPsndocVO[] psndocVOs = tbmpsndocService.queryPsnorgLatestByCondition(pk_org, fromWhereSQL, yearEarliestDay, yearLatesDay);
		//如果没有考勤档案，则返回null
		...
		LeaveBalanceVO[] vos =  leaveBalanceDAO.queryByCondition4HireDateSettlementWithHireDateTempTable(pk_org, typeVO, year, fromWhereSQL);
		return processMultiRecordsPerPsnorg(pk_org, typeVO, year, null, null, null, vos);
	}

	/**
	 * 处理一个pk_psnorg的所有可能的结算记录
	 * 逻辑是：首先将psnorg的vos按pk_psnorg分组，然后按pk_psnorg循环处理
	 * 如果存在未结算记录，则此pk_psnorg的结算数据是完整的，不用再处理（如果此未结算的记录pk_leavebalance为空，则将其leaveindex set 为1）
	 * 如果所有记录都已结算，则分两种情况：
	 * 1.如果最晚的结算日期>=期间最后一天，则不处理
	 * 2.如果最晚的结算日期<期间最后一天，则从最晚的结算日期+1天到期间最后一天内，pk_psnorg有考勤档案记录，则需要用代码new一条，且此条的leaveindex=最晚的结算日期的index+1
	 * @param pk_org
	 * @param typeVO
	 * @param year
	 * @param month，月份，如果是按年结算或者按入职日结算，此参数可以为null
	 * @param periodBeginDate,如果是按年结算或按月结算，则是年或者期间的第一天；若是按入职日结算，则传空即可
	 * @param periodEndDate,如果是按年结算或按月结算，则是年或者期间的最后一天；若是按入职日结算，则传空即可
	 * @param vos
	 * @throws BusinessException
	 */
	private LeaveBalanceVO[] processMultiRecordsPerPsnorg(String pk_org,LeaveTypeCopyVO typeVO,
			String year, String month,
			UFLiteralDate periodBeginDate,
			UFLiteralDate periodEndDate,LeaveBalanceVO[] vos) throws BusinessException{
		if(ArrayUtils.isEmpty(vos))
			return vos;
		String pk_group = InvocationInfoProxy.getInstance().getGroupId();
...
//		List<LeaveBalanceVO> voList = Arrays.asList(vos);
		...
		for(String pk_psnorg:psnorgGroupMap.keySet()){
			...
			if(unSetVO!=null){//如果存在未结算的数据，则不用考虑new一个新的
				continue;
			}
			...
			TBMPsndocVO latestTbmPsndocVO = tbmpsndocService.queryLatestByPsnorgDate(pk_org, pk_psnorg, beginDate, psnorgPeriodEndDate);
...
//		return vos.length==voList.size()?vos:voList.toArray(new LeaveBalanceVO[0]);
		return vos;
	}

	/**
	 * 适用于按年、期间结算的查询
	 * 查询的逻辑是，首先确定期间的开始、结束日期
	 * 数据规则是：pk_psnorg在期间范围内存在考勤档案记录，则此pk_psnorg将至少拥有一条结算记录，且最多有一条未结算的记录，且未结算的记录只能是最后一条（leaveindex最大的一条）
	 * 然后使用 select from 考勤档案 left join 假期结余表 来查询，查询的结果，肯定是每个pk_psnorg至少有一条记录（因为是left join）
	 ...
	 * @throws BusinessException
	 */
	protected LeaveBalanceVO[] queryByCondition4YearMonthSettlement(String pk_org,
			LeaveTypeCopyVO typeVO, String year, String month, FromWhereSQL fromWhereSQL)
	throws BusinessException {
		...
		LeaveBalanceVO[] vos = new LeaveBalanceDAO().queryByDateScope(pk_org, fromWhereSQL, periodBeginDate, periodEndDate, pk_leavetype, year, isYear?null:month);
		if(!ArrayUtils.isEmpty(vos)){
			UFLiteralDate priodExtendEndDate = typeVO.getExtendDaysCount()==0?periodEndDate:periodEndDate.getDateAfter(typeVO.getExtendDaysCount());
			for(LeaveBalanceVO vo:vos){
				vo.setPeriodbegindate(periodBeginDate);
				vo.setPeriodenddate(periodEndDate);
	 ...
	 * @throws BusinessException
	 */
	private UFLiteralDate[] queryPeriodBeginEndDate(String pk_org,LeaveTypeCopyVO typeVO,String year,String month,boolean throwsException) throws BusinessException{
		int leavesetperiod = typeVO.getLeavesetperiod().intValue();
...
/*@res "{0}年考勤期间未定义!"*/, year));
	 ...
	 * @throws BusinessException
	 */
	private UFLiteralDate[] queryPeriodBeginEndDate(String pk_org,LeaveTypeCopyVO typeVO,String year,String month) throws BusinessException{
...
//
//		LeaveBalanceVO[] vos = (LeaveBalanceVO[]) CommonMethods.toArray(LeaveBalanceVO.class, new BaseDAO().retrieveByClause(LeaveBalanceVO.class, cond, para));
//
//		return setDefaultTimeitemCopy(vos);
//	}
	...
	public LeaveBalanceVO[] queryByCondition(String pk_org,
		...
		LeaveTypeCopyVO typeVO =  (LeaveTypeCopyVO) service.queryCopyTypesByDefPK(pk_org,pk_leavetype,TimeItemCopyVO.LEAVE_TYPE);
		return queryByCondition(pk_org, typeVO, year, month, TBMPsndocSqlPiecer.addDeptPk2QuerySQL(pk_dept, containsSubDepts, fromWhereSQL));
	}
...
//	}
	...
	@Override
	public LeaveBalanceVO[] queryByCondition(LoginContext context,
			String pk_leavetype, String year, String month, FromWhereSQL fromWhereSQL)
			throws BusinessException {

		ITimeItemQueryService service = NCLocator.getInstance().lookup(ITimeItemQueryService.class);
		LeaveTypeCopyVO typeVO =  (LeaveTypeCopyVO) service.queryCopyTypesByDefPK(context.getPk_org(),pk_leavetype,TimeItemCopyVO.LEAVE_TYPE);
		//考虑权限
	...
	throws BusinessException {
		...
		if(leavesetperiod==LeaveTypeCopyVO.LEAVESETPERIOD_DATE){
			leaveBalanceVOs = setDefaultTimeitemCopy(queryByCondition4HireDateSettlement(pk_org, typeVO, year, fromWhereSQL));
		}else{
			leaveBalanceVOs = setDefaultTimeitemCopy(queryByCondition4YearMonthSettlement(pk_org, typeVO, year, month, fromWhereSQL));
		}
	...
	/**
	 * 针对按年或者期间结算的休假类别的第一次结算调用
	 * 这两种都是固定期间
	 * 如果结算日<期间首天，则抛异常
	 * 如果结算日>=有效期，则不用任何提示
	 * 也即，只有在结算日在期间首天至有效期的范围内，才有可能需要提示用户
	 * 如果结算日<=期间末天，则只可能提示考勤档案在结算日到期间末天不存在的人员
	 * 如果结算日在期间末天到有效期之内，则可能需要提示两种人员：a.结算日到有效期内无考勤档案的人员b.结算日到有效期内有考勤档案的人员，这两种人员用户可能要区别对待，因此要分别提示
	 * 综上所述，可能提示的有三种人员，但三种不可能同时出现
	 * @param salaryMonth 
	 * @param salaryYear 
	 */
	protected SettlementResult firstSettlement4YearMonth(String pk_org, LeaveTypeCopyVO typeVO,
			String year, String month, LeaveBalanceVO[] vos,
			UFLiteralDate settlementDate, String salaryYear, String salaryMonth) throws BusinessException {
		...
		SettlementGroupVO sgv = groupSettlementVOs4YearMonth(pk_org, typeVO, year, periodBeginDate, periodEndDate, vos, settlementDate);
		...
		if(ArrayUtils.isEmpty(canSettleVOs))
			return result;
		UFLiteralDate[] nextPeriodBeginEndDate = queryPeriodBeginEndDate(pk_org, typeVO, nextYearMonth[0], nextYearMonth.length==2?nextYearMonth[1]:null);
		...
		if(!ArrayUtils.isEmpty(previousYearMonth)){
			UFLiteralDate[] previousPeriodBeginEndDate = queryPeriodBeginEndDate(pk_org, typeVO, previousYearMonth[0], previousYearMonth.length==2?previousYearMonth[1]:null,false);
		...
		settlement(pk_org, typeVO, year, month,previousPeriod,nextPeriod,canSettleVOs,createLineNoMap(canSettleVOs), settlementDate,result,salaryYear,salaryMonth);
		return result;
	}
	/**
	 * 针对按入职日结算的休假类别的第一次结算调用
	 * 如果结算日<year的首天，则抛异常
	 * 如果结算日>year+1的末天，则不用任何提示
	 * 也即，只有在year首天到year+1的末天范围内，才有可能有提示
	 * 提示的人员分为三种：
	 * 1.还未到期末天，但结算日到期末天内已无考勤档案的人员
	 * 2.还未到有效期，但结算日到有效期已无考勤档案的人员
	 * 3.还未到有效期，结算日到有效期内还有考勤档案的人员
	 * @param salaryMonth 
	 * @param salaryYear 
	 */
	protected SettlementResult firstSettlement4HireDate(String pk_org, LeaveTypeCopyVO typeVO,
			String year, LeaveBalanceVO[] vos,
			UFLiteralDate settlementDate, String salaryYear, String salaryMonth) throws BusinessException {
		...
		settlement(pk_org, typeVO, year, null,previousPeriod,nextPeriod,canSettleVOs,createLineNoMap(canSettleVOs), settlementDate,result,salaryYear,salaryMonth);
		return result;
	}
	...
	@SuppressWarnings("unchecked")
	private void settlement(String pk_org, LeaveTypeCopyVO typeVO,
			String year, String month, 
			PeriodVO previousPeriod,PeriodVO nextPeriod,
			LeaveBalanceVO[] vos,
			Map<String, Integer> lineNoMap,//由于此方法传入的vos是界面vos的子集，为了正确地提示行号，需要将vo的行号放入map，key是pk_psnorg+leaveindex
			UFLiteralDate settlementDate,SettlementResult result, String salaryYear, String salaryMonth) throws BusinessException{
		if(ArrayUtils.isEmpty(vos))
			return;
		//结算前，需要先计算
		...
		String pk_timeitem = typeVO.getPk_timeitem();
		String periodField = isMonth?"curyear||curmonth":"curyear";
		String nextPeriodBadCond = "pk_org=? and pk_psnorg=? and pk_timeitem=? and (("+periodField+">? and "+LeaveBalanceVO.ISSETTLEMENT+"='Y') or ("+periodField+"=? and "+LeaveBalanceVO.LEAVEINDEX+">?))";
		String nextPeriodCond = "pk_org=? and pk_psnorg=? and pk_timeitem=? and "+periodField+"=? and "+LeaveBalanceVO.LEAVEINDEX+"=1";
		
		String preLeaveIndexBadCond = "pk_org=? and pk_psnorg=? and pk_timeitem=? and "+LeaveBalanceVO.ISSETTLEMENT+"='Y' and "+periodField+"=? and "+LeaveBalanceVO.LEAVEINDEX+"=?";
		...
		TBMPsndocVO[] existtbmpsndocs = previousPeriod==null?null:psndocService.queryTBMPsndocByPsnorgs(pk_org, StringPiecer.getStrArray(vos, LeaveBalanceVO.PK_PSNORG), previousPeriod.getBegindate(), previousPeriod.getEnddate());
		Map<Object, TBMPsndocVO[]> tbmpsnExistMap = CommonUtils.group2ArrayByField(TBMPsndocVO.PK_PSNORG, existtbmpsndocs);
		...
		for(LeaveBalanceVO vo:vos){
			...
			para.addParam(isMonth?(year+month):year);
			para.addParam(vo.getLeaveindex());
			LeaveBalanceVO[] badVOs = (LeaveBalanceVO[]) CommonUtils.toArray(LeaveBalanceVO.class, dao.retrieveByClause(LeaveBalanceVO.class, nextPeriodBadCond, para));
			if(!ArrayUtils.isEmpty(badVOs)){
//				errMsg.append(MessageFormat.format(getNextPeriodSealedError(), Integer.toString(lineNoMap.get(vo.getPk_psnorg()+vo.getLeaveindex())+1)));
				nextPeriodSealedLine.add(lineNoMap.get(vo.getPk_psnorg()+vo.getLeaveindex()));
				continue;
			...
			if(vo.getLeaveindex().intValue()>1){
				...
				badVOs = (LeaveBalanceVO[]) CommonUtils.toArray(LeaveBalanceVO.class, dao.retrieveByClause(LeaveBalanceVO.class, preLeaveIndexBadCond, para));
				if(ArrayUtils.isEmpty(badVOs)||!badVOs[0].isSettlement()){
//					errMsg.append(MessageFormat.format(getSamePreRecordNotSealedError(), Integer.toString(lineNoMap.get(vo.getPk_psnorg()+vo.getLeaveindex())+1)));
...
//					existsTBMPsndoc = psndocService.existsTBMPsndoc(pk_org, vo.getPk_psnorg(), previousPeriod.getBegindate(), previousPeriod.getEnddate());
					...
					if(psnOrgDateMap.get(vo.getPk_psnorg())==null || !psnOrgDateMap.get(vo.getPk_psnorg()).equals(vo.getHirebegindate()))
						existsTBMPsndoc = psndocService.existsTBMPsndoc(pk_org, vo.getPk_psnorg(), getHireBeginDate(previousPeriod.getYear(), vo.getHirebegindate()), vo.getHirebegindate().getDateBefore(1));
				}
				if(existsTBMPsndoc){
					para.clearParams();
					para.addParam(pk_org);
					para.addParam(vo.getPk_psnorg());
					para.addParam(pk_timeitem);
					para.addParam(isMonth?(previousPeriod.getTimeyear()+previousPeriod.getTimemonth()):previousPeriod.getTimeyear());
					badVOs = (LeaveBalanceVO[]) CommonUtils.toArray(LeaveBalanceVO.class, dao.retrieveByClause(LeaveBalanceVO.class, prePeriodCond, para));
...
//						errMsg.append(MessageFormat.format(getPreviousPeriodNotSealedError(), Integer.toString(lineNoMap.get(vo.getPk_psnorg()+vo.getLeaveindex())+1)));
			...
			if(leaveSet==LeaveTypeCopyVO.LEAVESETTLEMENT_NEXT){
				...
				if(leaveSetPeriod!=LeaveTypeCopyVO.LEAVESETPERIOD_DATE){//按年和期间结算的，下期的日期范围是固定的
					needInsert = psndocService.existsTBMPsndoc(pk_org, vo.getPk_psnorg(), nextPeriod.getBegindate(), nextPeriod.getEnddate());
				}
				else{//按入职日结算的，下期的日期范围因人而异
					needInsert = psndocService.existsTBMPsndoc(pk_org, vo.getPk_psnorg(), vo.getHireenddate().getDateAfter(1), getHireEndDate(nextPeriod.getTimeyear(), vo.getHirebegindate()));
				}
				
				LeaveBalanceVO[] nextPeriodVOs = (LeaveBalanceVO[]) CommonUtils.toArray(LeaveBalanceVO.class, dao.retrieveByClause(LeaveBalanceVO.class, nextPeriodCond, para));
				UFDouble toLeave = vo.getRestdayorhour();//转下期的数据，根据需求，转下期是有上限的，因此在此处理。
				if(typeVO.getLeavemax()!=null&&toLeave!=null&&typeVO.getLeavemax().doubleValue()>0&&toLeave.doubleValue()>typeVO.getLeavemax().doubleValue()){
					toLeave = typeVO.getLeavemax();
				}
				//如果数据库有，则需要update，否则需要insert
//				if(!ArrayUtils.isEmpty(nextPeriodVOs)){
				if(!ArrayUtils.isEmpty(nextPeriodVOs)&&needInsert){//2015-09-10修改，若下期没有考勤档案则结余不转下期，解决离职再入职时会把离职前的结余数据带进来（离职时可能已经结算工资了）
					LeaveBalanceVO updateVO = nextPeriodVOs[0];
					updateList.add(updateVO);
					updateVO.setLastdayorhour(toLeave);//将本期结余更新到下期的“上期结余”
					if(toLeave!=null&&toLeave.doubleValue()!=0){//如果本期结余不为0，才有必要重算下期的结余
						updateVO.setRestdayorhour(updateVO.getRestdayorhour()==null?toLeave:toLeave.add(updateVO.getRestdayorhour()));
					}
				}
				else{
//					//是否要insert，需要看下期是否有考勤档案记录，如果没有，则不需要insert
...
//						nextPeriodVO.setRestdayorhour(vo.getRestdayorhour());
	 ...
	 * @throws BusinessException
	 */
	private String[] queryNextPeriod(String pk_org,LeaveTypeCopyVO typeVO,String year,String month) throws BusinessException{
		int leaveSetPeriod = typeVO.getLeavesetperiod().intValue();
	 ...
	 * @throws BusinessException
	 */
	private String[] queryPreviousPeriod(String pk_org,LeaveTypeCopyVO typeVO,String year,String month) throws BusinessException{
		int leaveSetPeriod = typeVO.getLeavesetperiod().intValue();
		if(leaveSetPeriod==TimeItemCopyVO.LEAVESETPERIOD_MONTH){
	...
	/*
	 * 调用到这个方法的时候，说明界面上一定有未到结算有效期，或者未到结算日但已无考勤档案的人员，已经在界面上提示过
	 * (non-Javadoc)
	 * @see nc.itf.ta.ILeaveBalanceManageMaintain#secondSettlement(java.lang.String, java.lang.String, java.lang.String, java.lang.String, nc.vo.ta.leavebalance.LeaveBalanceVO[], nc.vo.pub.lang.UFLiteralDate, boolean, boolean)
	 */
	@Override
	public SettlementResult secondSettlement(String pk_org,String pk_leavetype,String year,String month,
		...
		UFLiteralDate settlementDate = PubEnv.getServerLiteralDate();
		return secondSettlement(pk_org, pk_leavetype, year, month, vos, settlementDate, isSettleNotToPeriodEndDateNoTbmPsndoc, isSettleNotToEffectiveDateNoTbmPsndocVOs, isSettleNotToEffectiveDateExistTbmPsndocVOs, needCheckNextPeriod, pk_salaryPeriod);
	}
	...
	private SettlementResult secondSettlement(String pk_org,String pk_leavetype,String year,String month,
		...
		if(leavesetPeriod==TimeItemCopyVO.LEAVESETPERIOD_DATE)
			return secondSettlement4HireDate(pk_org, typeVO, year, vos, settlementDate, isSettleNotToPeriodEndDateNoTbmPsndoc, isSettleNotToEffectiveDateNoTbmPsndocVOs, isSettleNotToEffectiveDateExistTbmPsndocVOs,needCheckNextPeriod,salaryYear,salaryMonth);
		return secondSettlement4YearMonth(pk_org, typeVO, year, month, vos, settlementDate, isSettleNotToPeriodEndDateNoTbmPsndoc, isSettleNotToEffectiveDateNoTbmPsndocVOs, isSettleNotToEffectiveDateExistTbmPsndocVOs,needCheckNextPeriod,salaryYear,salaryMonth);
	}

	protected SettlementResult secondSettlement4YearMonth(String pk_org, LeaveTypeCopyVO typeVO,
			String year, String month, LeaveBalanceVO[] vos,
			UFLiteralDate settlementDate,
			boolean isSettleNotToPeriodEndDateNoTbmPsndoc,
			boolean isSettleNotToEffectiveDateNoTbmPsndocVOs,
			boolean isSettleNotToEffectiveDateExistTbmPsndocVOs, boolean needCheckNextPeriod, String salaryYear, String salaryMonth) throws BusinessException {
		UFLiteralDate[] periodBeginEndDate = queryPeriodBeginEndDate(pk_org, typeVO, year, month);
		UFLiteralDate periodBeginDate = periodBeginEndDate[0];
		UFLiteralDate periodEndDate = periodBeginEndDate[1];
		SettlementGroupVO sgv = groupSettlementVOs4YearMonth(pk_org, typeVO, year, periodBeginDate, periodEndDate, vos, settlementDate);
		LeaveBalanceVO[] canSettleVOs = sgv.toCanSettleVOs(isSettleNotToPeriodEndDateNoTbmPsndoc, isSettleNotToEffectiveDateNoTbmPsndocVOs, isSettleNotToEffectiveDateExistTbmPsndocVOs);
		SettlementResult result = new SettlementResult();
		result.setSettledVOs(vos);
		if(ArrayUtils.isEmpty(canSettleVOs))
			return result;
	...
	protected SettlementResult secondSettlement4HireDate(String pk_org, LeaveTypeCopyVO typeVO,
			String year, LeaveBalanceVO[] vos,
			UFLiteralDate settlementDate,
			boolean isSettleNotToPeriodEndDateNoTbmPsndoc,
			boolean isSettleNotToEffectiveDateNoTbmPsndocVOs,
	 ...
	 * @throws BusinessException
	 */
	private SettlementGroupVO groupSettlementVOs4HireDate(String pk_org, LeaveTypeCopyVO typeVO,
			String year, LeaveBalanceVO[] vos,//要求传入的vo已经和数据库中的同步过
		...
		if(!PubEnv.UAP_USER.equalsIgnoreCase(pk_user))//2013-03-29后台任务查询权限报错
			perimssionMap = perimssionService.isUserhasPermissionByMetaDataOperation("60170psndoc",StringPiecer.getStrArrayDistinct(vos, LeaveBalanceVO.PK_TBM_PSNDOC), "LeaveBalanceAction", pk_group, pk_user);
		for(LeaveBalanceVO vo:vos){
...
//					vo.toTBMPsndocVO().getPrimaryKey(), "LeaveBalanceAction", pk_group, pk_user))
//				continue;//没有结算权限的，刨除掉
			if(!settlementDate.before(vo.getHirebegindate())&&!settlementDate.after(vo.getHireenddate())){
				boolean existPsndoc = tbmpsndocService.existsTBMPsndoc(pk_org, vo.getPk_psnorg(), settlementDate.getDateAfter(1), vo.getHireenddate());
				if(!existPsndoc){
			...
			UFLiteralDate effectiveDate = extendCount==0?vo.getHireenddate():vo.getHireenddate().getDateAfter(extendCount);
			if(settlementDate.after(vo.getHireenddate())&&!settlementDate.after(effectiveDate)){
				boolean existPsndoc = tbmpsndocService.existsTBMPsndoc(pk_org, vo.getPk_psnorg(), settlementDate.getDateAfter(1), effectiveDate);
				if(existPsndoc){
		...
		if(notToPeriodEndDateNoTbmPsndocList.size()>0)retVO.notToPeriodEndDateNoTbmPsndocVOs=notToPeriodEndDateNoTbmPsndocList.toArray(new LeaveBalanceVO[0]);
		if(notToEffectiveDateNoTbmPsndocList.size()>0)retVO.notToEffectiveDateNoTbmPsndocVOs=notToEffectiveDateNoTbmPsndocList.toArray(new LeaveBalanceVO[0]);
		if(notToEffectiveDateExistTbmPsndocList.size()>0)retVO.notToEffectiveDateExistTbmPsndocVOs=notToEffectiveDateExistTbmPsndocList.toArray(new LeaveBalanceVO[0]);
		if(exceedEffectiveDateList.size()>0)retVO.exceedEffectiveDateVOs=exceedEffectiveDateList.toArray(new LeaveBalanceVO[0]);
		return retVO;
	}
	 ...
	 * @throws BusinessException
	 */
	private SettlementGroupVO groupSettlementVOs4YearMonth(String pk_org, LeaveTypeCopyVO typeVO,
			String year, UFLiteralDate periodBeginDate,UFLiteralDate periodEndDate,
		...
		String pk_group = InvocationInfoProxy.getInstance().getGroupId();
		IDataPermissionPubService perimssionService = NCLocator.getInstance().lookup(IDataPermissionPubService.class);
		Map<String, UFBoolean> perimssionMap = perimssionService.isUserhasPermissionByMetaDataOperation("60170psndoc",StringPiecer.getStrArrayDistinct(vos, LeaveBalanceVO.PK_TBM_PSNDOC), "LeaveBalanceAction", pk_group, pk_user);
		if(settlementDate.after(effectiveDate)){//如果结算日已过有效期，则只可能有两种：结算了的，和剩下的
			...
			if(leftList.size()>0)retVO.exceedEffectiveDateVOs=leftList.toArray(new LeaveBalanceVO[0]);
			return retVO;
		}
		ITBMPsndocQueryService tbmpsndocService = NCLocator.getInstance().lookup(ITBMPsndocQueryService.class);
		...
		if(!settlementDate.after(periodEndDate)){
			...
			//优化时添加
			TBMPsndocVO[] tbmpsndocs = tbmpsndocService.queryTBMPsndocByPsnorgs(pk_org, StringPiecer.getStrArray(vos, LeaveBalanceVO.PK_PSNORG), settlementDate.getDateAfter(1), periodEndDate);
			...
			for(LeaveBalanceVO vo:vos){
				...
				if(perimssionMap!=null && perimssionMap.get(vo.toTBMPsndocVO().getPrimaryKey())!=null && 
					...
					continue;//没有结算权限的，刨除掉
				//不再循环中查询了，提取出来
//				boolean existPsndoc = tbmpsndocService.existsTBMPsndoc(pk_org, vo.getPk_psnorg(), settlementDate.getDateAfter(1), periodEndDate);
//				if(!existPsndoc){
//					noPsndocList.add(vo);
//					continue;
//				}
			...
			if(noPsndocList.size()>0)retVO.notToPeriodEndDateNoTbmPsndocVOs=noPsndocList.toArray(new LeaveBalanceVO[0]);
			return retVO;
		}
		...
		if(settlementDate.after(periodEndDate)&&!settlementDate.after(effectiveDate)){
			...
			//优化时添加
			TBMPsndocVO[] tbmpsndocs = tbmpsndocService.queryTBMPsndocByPsnorgs(pk_org, StringPiecer.getStrArray(vos, LeaveBalanceVO.PK_PSNORG), settlementDate.getDateAfter(1), effectiveDate);
			Map<Object, TBMPsndocVO[]> tbmpsnMap = CommonUtils.group2ArrayByField(TBMPsndocVO.PK_PSNORG, tbmpsndocs);
			boolean mapIsNull = MapUtils.isEmpty(tbmpsnMap);
...
//				}
			...
			if(existPsndocList.size()>0)retVO.notToEffectiveDateExistTbmPsndocVOs=existPsndocList.toArray(new LeaveBalanceVO[0]);
			if(noPsndocList.size()>0)retVO.notToEffectiveDateNoTbmPsndocVOs=noPsndocList.toArray(new LeaveBalanceVO[0]);
			return retVO;
		}
		return retVO;
	}
...
//		LeaveBalanceVO[] notToPeriodEndDateExistTbmPsndocVOs;//未结算，且未到期末日，且结算日到期末日有考勤档案的人员，无需提示，肯定不能结算
		...
		LeaveBalanceVO[] toCanSettleVOs(boolean isSettleNotToPeriodEndDateNoTbmPsndoc,
			...
			if(isSettleNotToPeriodEndDateNoTbmPsndoc&&!ArrayUtils.isEmpty(notToPeriodEndDateNoTbmPsndocVOs))
				retVOs = (LeaveBalanceVO[]) org.apache.commons.lang.ArrayUtils.addAll(retVOs, notToPeriodEndDateNoTbmPsndocVOs);
			if(isSettleNotToEffectiveDateNoTbmPsndocVOs&&!ArrayUtils.isEmpty(notToEffectiveDateNoTbmPsndocVOs))
				retVOs = (LeaveBalanceVO[]) org.apache.commons.lang.ArrayUtils.addAll(retVOs, notToEffectiveDateNoTbmPsndocVOs);
			if(isSettleNotToEffectiveDateExistTbmPsndocVOs&&!ArrayUtils.isEmpty(notToEffectiveDateExistTbmPsndocVOs))
				retVOs = (LeaveBalanceVO[]) org.apache.commons.lang.ArrayUtils.addAll(retVOs, notToEffectiveDateExistTbmPsndocVOs);
			return retVOs;
		}
	}
	...
	private LeaveBalanceVO[] setDefaultTimeitemCopy(LeaveBalanceVO[] vos) throws BusinessException{
		...
		for(int i=0; i<vos.length; i++){
			...
			String pk_timeitemcopy = allTimeitemMap.get(pk_org)==null?null:allTimeitemMap.get(pk_org).get(vos[i].getPk_timeitem()).getPk_timeitemcopy();
	...
	public LeaveBalanceVO[] queryByPsn(String pk_psndoc, String pk_leavetype,
			String year, boolean containsHis,UFBoolean issettlement) throws BusinessException {
		TBMPsndocVO latestVO = NCLocator.getInstance().lookup(ITBMPsndocQueryMaintain.class).queryByPsndocAndDateTime(pk_psndoc, new UFDateTime());
		...
		LeaveTypeCopyVO typeCopyVO = (LeaveTypeCopyVO) timeItemService.queryCopyTypesByDefPK(latestVO.getPk_org(), pk_leavetype, TimeItemVO.LEAVE_TYPE);
	 ...
	 * @throws BusinessException
	 */
	private String processPsnjobAndLeaveTypeCopy4Psn(String pk_psndoc,String pk_leavetype,LeaveBalanceVO[] leaveBalanceVOs ) throws BusinessException{
		ITBMPsndocQueryService psndocService = NCLocator.getInstance().lookup(ITBMPsndocQueryService.class);
		ITimeItemQueryService timeItemService = NCLocator.getInstance().lookup(ITimeItemQueryService.class);
		...
		GeneralVO[] pk_timeitems = (GeneralVO[]) new BaseDAO().executeQuery(Sql, new GeneralVOProcessor<GeneralVO>(GeneralVO.class));		
		for(LeaveBalanceVO leaveBalanceVO:leaveBalanceVOs){
			...
			if(pk_leavetype!=null && pk_leavetype.equals(pk_timeitems==null?null:pk_timeitems[0].getAttributeValue("pk_timeitem"))){
				pk_psnorg = psndocVO.getPk_psnorg();
			}
			LeaveTypeCopyVO typeCopyVO = (LeaveTypeCopyVO) timeItemService.queryCopyTypesByDefPK(pk_org, pk_leavetype, TimeItemVO.LEAVE_TYPE);
...
/*@res "系统未定义下期!"*/;
	...
	public void translateLeave(PsnJobVO oldJobvo,PsnJobVO newJobvo)throws BusinessException {
		...
		for(String pk_timeitem:oldMap.keySet()){
			...
			//查询原组织记录
			LeaveBalanceVO[] oldLeaveBalanceVOs = queryByCondition(oldhrorg, oldTypeVO, oldPeriod.getTimeyear(), oldPeriod.getTimemonth(), oldFromWhereSQL);
...
//			calculate(oldhrorg, oldTypeVO, oldPeriod.getTimeyear(), oldPeriod.getTimemonth(),oldLeaveBalanceVOs, new UFDateTime(enddate.toDate()));
			//结算，获取结余时长
			SettlementResult secondSettlement = secondSettlement(oldhrorg, pk_timeitem, oldPeriod.getTimeyear(), oldPeriod.getTimemonth(), new LeaveBalanceVO[]{oldLeaveVO}, enddate, true, true, true,false,null);
...
//			oldSettVO.setRealdayorhour(new UFDouble(0));
			...
			//计算时间的不能大于结算时间
//			if(oldSettVO.getCalculatetime().after(new UFDateTime(oldSettVO.getSettlementdate().toDate()))){
//				oldSettVO.setCalculatetime(new UFDateTime(oldSettVO.getSettlementdate().toDate()));
//			}
			...
			//获取转入组织的记录
			LeaveBalanceVO[] newLeaveBalanceVOs = queryByCondition(newhrorg, newTypeVO, newPeriod.getTimeyear(), newPeriod.getTimemonth(), newFromWhereSQL);
			//计算
//			newLeaveBalanceVOs = calculate(newhrorg, newTypeVO, newPeriod.getTimeyear(), newPeriod.getTimemonth(), newLeaveBalanceVOs, new UFDateTime(enddate.toDate()));
...
//			newVo.setRestdayorhour(newVo.getRestdayorhour().add(inLength));
			calculate(newhrorg, newTypeVO, newPeriod.getTimeyear(), newPeriod.getTimemonth(), new LeaveBalanceVO[]{newVo}, new UFDateTime(enddate.toDate()));
...
//			newList.add(newVo);
	...
	@Override
	public LeaveBalanceVO[] queryByPsnPks(String[] pk_psnOrgs,LoginContext context,String pk_leavetype,String year,String month)
			throws BusinessException {
		//使用pk_psndocs对离职再入职人员会出现一个Pk_psndoc对应两条数据的情况，修改为使用pk_psnorg
//		FromWhereSQL createPsndocArrayQuerySQL = TBMPsndocSqlPiecer.createPsndocArrayQuerySQL(pk_psndocs);
		FromWhereSQL fromWhereSql = TBMPsndocSqlPiecer.addTBMPsndocCond2QuerySQL(" pk_psnorg in(" + new InSQLCreator().getInSQL(pk_psnOrgs)+") ", null);
		return queryByCondition(context, pk_leavetype, year, month, fromWhereSql);
	}
